package com.markus.cryptoappcleanarchitecture.domain.use_case.get_coin

import com.markus.cryptoappcleanarchitecture.common.Resource
import com.markus.cryptoappcleanarchitecture.data.remote.dto.toCoinDetail
...
class GetCoinUseCase @Inject constructor( //Inject our repository/fake repository not Impl
    private val repository: CoinRepository
) {
    operator fun invoke(coinId: String): Flow<Resource<CoinDetail>> = flow {
        try {
            emit(Resource.Loading())
            val coin = repository.getCoinById(coinId).toCoinDetail() //map our coinDetailDto to CoinDetail
